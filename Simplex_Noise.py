...
class Simplex_Noise:
    ...
    def __init__(self, frequency, amplitude, start_x, stop_x, start_y, stop_y, res, seed, 
        ...
        if self.warp_noise_amp > 0.0:
            ...
            self.Wu.fBm(True, octaves=warp_noise_octaves)
            self.Wv.fBm(True, octaves=warp_noise_octaves)
    ...
    def fBm(self, octaves=4, lacunarity=2, persistence=0.5, octave_amplitude_array = None, fBm_bias_function=None):      # Fractional Brownian Motion for multiple octaves
        ...
        for n in range(1, octaves + 1):
            ...
            S = Simplex_Noise(S_freq, S_ampl, start_x, stop_x, start_y, stop_y, self.res, self.seed + n, self.warp_x, self.warp_y, warp_noise_amplitude=S_warp_ampl, warp_noise_freq=S_warp_freq, warp_noise_seed=self.warp_noise_seed, 
                    warp_noise_octaves=self.warp_noise_octaves, function=fBm_bias_function, rotation_radians=self.rotation_radians + 1.3)
...
if __name__ == "__main__":
    ...
    resolution = int(area ** 0.5) * int(res) # Resolution 

    bias_func = x                                        # Input a function in terms of x for Sympy
    #bias_func = sp.sqrt(x ** 2 + 0.000001)* -1          # Ridged noise
    #bias_func = sp.sin(x**3 - 5 * x**2 + x)             # Something crazy

    S1 = Simplex_Noise(1 / freq, amplitude, start_x, stop_x, start_y, stop_y, resolution, seed, function=bias_func, rotation_radians=1)

    """warp_x = sp.sqrt((x**2 + y**2) + 0.00001)
    warp_y = sp.sqrt((x**2 + y**2) + 0.00001) * -1
    S1 = Simplex_Noise(1 / freq, amplitude, start_x, stop_x, start_y, stop_y, resolution, seed, warp_x, warp_y, function=bias_func, rotation_radians=1) """ # Something crazier

    S1.fBm(10, persistence=0.75, lacunarity=1.5, fBm_bias_function=bias_func)
    S1.unlerp_noise(-1000, 1000)    # Controls value range of noise
    lighting_vector = [1, 1, 1]     # Direction of light in 3D space
    L = (shadow_map(S1.noisemap, lighting_vector, 1 / res, 100 * res, 0.5 / res) + lambertian_Lighting(S1, lighting_vector) / 5 + 0.2).get()
    L = (L - cp.min(L)) / (cp.max(L) - cp.min(L))

    Z = S1.get_NoiseMap().get()
    G = S1.gradient_Magnitude_Map().get()


    plt.imshow(Z, extent=(start_x, stop_x, start_y, stop_y), origin="lower", cmap="viridis")
    ...
    plt.title('Noise Map with Gradient Magnitude Overlay')
